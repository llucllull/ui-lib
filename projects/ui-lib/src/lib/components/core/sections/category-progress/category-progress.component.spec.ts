...
import { CategoryProgressComponent, ProgressCategoryI, ProgressItemI } from "./category-progress.component";
...
describe('CategoryProgressComponent', () => {
    ...
    describe('categories input', () => {
      ...
      it('debería renderizar tantas categorías como se pasen en @Input categories', () => {
        ...
        // Primera categoría
        const firstCategoryTitle = categories[0].query(By.css('.subtitle')).nativeElement.textContent;
        ...
        const firstItemLabel = firstCategoryItems[0].query(By.css('.label')).nativeElement.textContent;
        expect(firstItemLabel).toContain('Angular');
  
        const firstItemFill = firstCategoryItems[0].query(By.css('.fill')).nativeElement as HTMLElement;
